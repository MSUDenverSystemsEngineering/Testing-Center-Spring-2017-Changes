...
Try {
	...
	[string]$appName = 'Testing Center Updates'
	[string]$appVersion = 'Spring 2017'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '02/28/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'iexplore,firefox,winword,excel,powerpnt' -CheckDiskSpace -PersistPrompt

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		## Get local users directory
		#$UsersDirectory = "${envSystemDrive}\Users\"

		## Create an array of known local users and special folders
		$UsersToKeep = @("Level1","Level2","Testing","Engineering","Accesstesting16","PublicStation","Infotech","Administrator","All Users","Default","Default User","Public")
		...
		## Add password verification shortcut to desktop

		Foreach ($TestingUser in $UsersToKeep[0..1]) {
		Copy-File -Path "$dirSupportFiles\Password Verification.lnk" -Destination "$envSystemDrive\Users\$TestingUser\Desktop"
		}

		## Update Firefox profile path for Level1 and Level2
		Foreach ($TestingUser in $UsersToKeep[0..2])
		{
		Copy-File -Path "$dirSupportFiles\profiles.ini" -Destination "$envSystemDrive\Users\$TestingUser\AppData\Roaming\Mozilla\Firefox"
		}

		## Remove Send to OneNote printer
		Get-WmiObject Win32_Printer | Where-Object {$_.Name -eq 'Send To OneNote 2010'} | ForEach-Object {
			Write-Log -Message "Removing $_.Name" -Severity 1 -Source $deployAppScriptFriendlyName
			$_.delete()
		}

		Get-WmiObject Win32_Printer | Where-Object {$_.Name -eq 'Microsoft XPS Document Writer'} | ForEach-Object {
			Write-Log -Message "Removing $_.Name" -Severity 1 -Source $deployAppScriptFriendlyName
			$_.delete()
		}

		Get-WmiObject Win32_Printer | Where-Object {$_.Name -eq 'Fax'} | ForEach-Object {
			Write-Log -Message "Removing $_.Name" -Severity 1 -Source $deployAppScriptFriendlyName
			$_.delete()
		}
		...
		## Clean up default user desktop
		Remove-File -Path "$envSystemDrive\Users\Default\Desktop\*in Firefox.*" -ContinueOnError $true
		Remove-File -Path "$envSystemDrive\Users\Default\Desktop\*in Internet Explorer.*" -ContinueOnError $true

		## Clean up domain users
		#Get-ChildItem -Path "${envSystemDrive}\Users" -Force | ?{ $_.PSIsContainer } | Where-Object { $UsersToKeep -inotcontains $_.Name} | ForEach-Object {
		#Remove-File -Path "$_.FullName" -Recurse -ContinueOnError $true
		#Remove-Folder -Path "$_.FullName" -ContinueOnError $true
		#}

		## Remove ProfileList entries for domain users
		#Remove-RegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList\S-1-5-21-898739304*" -ContinueOnError

		#Get-ChildItem -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList" | Foreach-object {
		#	If ((Get-RegistryKey -Key "$_.Name" -Value "ProfileImagePath") -ieq )
		#}
		...
		## Clean up Infotech account
		Remove-File -Path "$envSystemDrive\Users\Infotech\Desktop\*" -Recurse -ContinueOnError $true
		Remove-File -Path "$envSystemDrive\Users\Infotech\Downloads\*" -Recurse -ContinueOnError $true
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'iexplore,firefox,winword,excel,powerpnt' -CloseAppsCountdown 60
